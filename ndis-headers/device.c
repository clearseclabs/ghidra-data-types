...
#pragma NDIS_INIT_FUNCTION(ndisheadersRegisterDevice)


_IRQL_requires_max_(PASSIVE_LEVEL)
NDIS_STATUS
ndisheadersRegisterDevice(
    VOID
    )
{
    ...
    DEBUGP(DL_TRACE, "==>ndisheadersRegisterDevice\n");
   
    
    NdisZeroMemory(DispatchTable, (IRP_MJ_MAXIMUM_FUNCTION+1) * sizeof(PDRIVER_DISPATCH));
    
    DispatchTable[IRP_MJ_CREATE] = ndisheadersDispatch;
    DispatchTable[IRP_MJ_CLEANUP] = ndisheadersDispatch;
    DispatchTable[IRP_MJ_CLOSE] = ndisheadersDispatch;
    DispatchTable[IRP_MJ_DEVICE_CONTROL] = ndisheadersDeviceIoControl;
    ...
    DEBUGP(DL_TRACE, "<==ndisheadersRegisterDevice: %x\n", Status);
